"""Evaluate STT accuracy against real-world clips.
...
Usage:
    python tests/stt_real_world/evaluate.py
...
async def main() -> None:
    if not MANIFEST.exists():
        ...
        print("Add clips to the manifest and record real-world audio first.")
        return
    ...
    for clip in clips:
        ...
        if r["wer"] is not None:
            status = "PASS" if r["wer"] < 0.15 else "FAIL"
            print(
    ...
    if valid:
        ...
        by_cond = {}
        for r in valid:
            by_cond.setdefault(r["category"], []).append(r["wer"])

        print(f"\n{'='*60}")
        print(f"AGGREGATE: {len(valid)}/{len(clips)} clips evaluated")
        print(f"  Average WER: {avg_wer:.1%}")
        print(f"  Max WER:     {max_wer:.1%}")
        print("  Target:      <15% (relaxed for real-world)")
        print(f"  Status:      {'PASS' if avg_wer < 0.15 else 'FAIL'}")

        print("\n  By condition:")
        for cond, wers in sorted(by_cond.items()):
            print(f"    {cond}: avg={sum(wers)/len(wers):.1%} (n={len(wers)})")
    ...
    report = {
        ...
        "target_wer": 0.15,
        "passed": avg_wer < 0.15 if valid else False,
        "results": results,
    }
